/* Created by Pgen */
package pers.xia.jpython.grammar;

import java.util.Map;
import java.util.HashMap;

import pers.xia.jpython.tokenizer.TokState;

public final class GramInit{

    public static Arc[] arcs_0_0 = {
        new Arc(4, 1),
        new Arc(2, 2),
        new Arc(3, 2),
    };

    public static Arc[] arcs_0_1 = {
        new Arc(2, 2),
    };

    public static Arc[] arcs_0_2 = {
        new Arc(1, -1),
    };

    public static State[] states_0 = {
        new State(3, GramInit.arcs_0_0),
        new State(1, GramInit.arcs_0_1),
        new State(1, GramInit.arcs_0_2),
    };

    public static Arc[] arcs_1_0 = {
        new Arc(2, 2),
        new Arc(7, 1),
        new Arc(6, 2),
    };

    public static Arc[] arcs_1_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_1_2 = {
        new Arc(2, 2),
        new Arc(7, 1),
        new Arc(6, 2),
    };

    public static State[] states_1 = {
        new State(3, GramInit.arcs_1_0),
        new State(1, GramInit.arcs_1_1),
        new State(3, GramInit.arcs_1_2),
    };

    public static Arc[] arcs_2_0 = {
        new Arc(9, 2),
    };

    public static Arc[] arcs_2_1 = {
        new Arc(2, 1),
        new Arc(7, 3),
    };

    public static Arc[] arcs_2_2 = {
        new Arc(2, 1),
        new Arc(7, 3),
    };

    public static Arc[] arcs_2_3 = {
        new Arc(1, -1),
    };

    public static State[] states_2 = {
        new State(1, GramInit.arcs_2_0),
        new State(2, GramInit.arcs_2_1),
        new State(2, GramInit.arcs_2_2),
        new State(1, GramInit.arcs_2_3),
    };

    public static Arc[] arcs_3_0 = {
        new Arc(11, 5),
    };

    public static Arc[] arcs_3_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_3_2 = {
        new Arc(15, 3),
    };

    public static Arc[] arcs_3_3 = {
        new Arc(2, 1),
    };

    public static Arc[] arcs_3_4 = {
        new Arc(14, 2),
        new Arc(15, 3),
    };

    public static Arc[] arcs_3_5 = {
        new Arc(12, 6),
    };

    public static Arc[] arcs_3_6 = {
        new Arc(2, 1),
        new Arc(13, 4),
    };

    public static State[] states_3 = {
        new State(1, GramInit.arcs_3_0),
        new State(1, GramInit.arcs_3_1),
        new State(1, GramInit.arcs_3_2),
        new State(1, GramInit.arcs_3_3),
        new State(2, GramInit.arcs_3_4),
        new State(1, GramInit.arcs_3_5),
        new State(2, GramInit.arcs_3_6),
    };

    public static Arc[] arcs_4_0 = {
        new Arc(10, 1),
    };

    public static Arc[] arcs_4_1 = {
        new Arc(1, -1),
        new Arc(10, 1),
    };

    public static State[] states_4 = {
        new State(1, GramInit.arcs_4_0),
        new State(2, GramInit.arcs_4_1),
    };

    public static Arc[] arcs_5_0 = {
        new Arc(16, 1),
    };

    public static Arc[] arcs_5_1 = {
        new Arc(18, 2),
        new Arc(19, 2),
        new Arc(20, 2),
    };

    public static Arc[] arcs_5_2 = {
        new Arc(1, -1),
    };

    public static State[] states_5 = {
        new State(1, GramInit.arcs_5_0),
        new State(3, GramInit.arcs_5_1),
        new State(1, GramInit.arcs_5_2),
    };

    public static Arc[] arcs_6_0 = {
        new Arc(21, 2),
    };

    public static Arc[] arcs_6_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_6_2 = {
        new Arc(19, 1),
    };

    public static State[] states_6 = {
        new State(1, GramInit.arcs_6_0),
        new State(1, GramInit.arcs_6_1),
        new State(1, GramInit.arcs_6_2),
    };

    public static Arc[] arcs_7_0 = {
        new Arc(22, 7),
    };

    public static Arc[] arcs_7_1 = {
        new Arc(28, 6),
    };

    public static Arc[] arcs_7_2 = {
        new Arc(24, 4),
    };

    public static Arc[] arcs_7_3 = {
        new Arc(26, 5),
    };

    public static Arc[] arcs_7_4 = {
        new Arc(25, 3),
        new Arc(27, 1),
    };

    public static Arc[] arcs_7_5 = {
        new Arc(27, 1),
    };

    public static Arc[] arcs_7_6 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_7_7 = {
        new Arc(23, 2),
    };

    public static State[] states_7 = {
        new State(1, GramInit.arcs_7_0),
        new State(1, GramInit.arcs_7_1),
        new State(1, GramInit.arcs_7_2),
        new State(1, GramInit.arcs_7_3),
        new State(2, GramInit.arcs_7_4),
        new State(1, GramInit.arcs_7_5),
        new State(1, GramInit.arcs_7_6),
        new State(1, GramInit.arcs_7_7),
    };

    public static Arc[] arcs_8_0 = {
        new Arc(13, 3),
    };

    public static Arc[] arcs_8_1 = {
        new Arc(15, 2),
    };

    public static Arc[] arcs_8_2 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_8_3 = {
        new Arc(15, 2),
        new Arc(29, 1),
    };

    public static State[] states_8 = {
        new State(1, GramInit.arcs_8_0),
        new State(1, GramInit.arcs_8_1),
        new State(1, GramInit.arcs_8_2),
        new State(2, GramInit.arcs_8_3),
    };

    public static Arc[] arcs_9_0 = {
        new Arc(34, 20),
        new Arc(33, 24),
        new Arc(30, 6),
    };

    public static Arc[] arcs_9_1 = {
        new Arc(26, 21),
    };

    public static Arc[] arcs_9_2 = {
        new Arc(1, -1),
        new Arc(31, 27),
        new Arc(32, 14),
    };

    public static Arc[] arcs_9_3 = {
        new Arc(1, -1),
        new Arc(32, 29),
    };

    public static Arc[] arcs_9_4 = {
        new Arc(1, -1),
        new Arc(34, 32),
        new Arc(30, 5),
        new Arc(33, 30),
    };

    public static Arc[] arcs_9_5 = {
        new Arc(1, -1),
        new Arc(31, 9),
        new Arc(32, 17),
    };

    public static Arc[] arcs_9_6 = {
        new Arc(1, -1),
        new Arc(31, 13),
        new Arc(32, 4),
    };

    public static Arc[] arcs_9_7 = {
        new Arc(1, -1),
        new Arc(32, 14),
    };

    public static Arc[] arcs_9_8 = {
        new Arc(1, -1),
        new Arc(31, 28),
        new Arc(32, 29),
    };

    public static Arc[] arcs_9_9 = {
        new Arc(26, 21),
    };

    public static Arc[] arcs_9_10 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_9_11 = {
        new Arc(1, -1),
        new Arc(32, 18),
    };

    public static Arc[] arcs_9_12 = {
        new Arc(1, -1),
        new Arc(31, 1),
        new Arc(32, 17),
    };

    public static Arc[] arcs_9_13 = {
        new Arc(26, 25),
    };

    public static Arc[] arcs_9_14 = {
        new Arc(34, 15),
        new Arc(30, 23),
    };

    public static Arc[] arcs_9_15 = {
        new Arc(30, 10),
    };

    public static Arc[] arcs_9_16 = {
        new Arc(26, 3),
    };

    public static Arc[] arcs_9_17 = {
        new Arc(1, -1),
        new Arc(34, 32),
        new Arc(30, 12),
        new Arc(33, 30),
    };

    public static Arc[] arcs_9_18 = {
        new Arc(34, 26),
        new Arc(30, 8),
    };

    public static Arc[] arcs_9_19 = {
        new Arc(1, -1),
        new Arc(31, 16),
        new Arc(32, 29),
    };

    public static Arc[] arcs_9_20 = {
        new Arc(30, 10),
    };

    public static Arc[] arcs_9_21 = {
        new Arc(1, -1),
        new Arc(32, 17),
    };

    public static Arc[] arcs_9_22 = {
        new Arc(1, -1),
        new Arc(32, 33),
    };

    public static Arc[] arcs_9_23 = {
        new Arc(1, -1),
        new Arc(31, 31),
        new Arc(32, 14),
    };

    public static Arc[] arcs_9_24 = {
        new Arc(1, -1),
        new Arc(32, 33),
        new Arc(30, 22),
    };

    public static Arc[] arcs_9_25 = {
        new Arc(1, -1),
        new Arc(32, 4),
    };

    public static Arc[] arcs_9_26 = {
        new Arc(30, 10),
    };

    public static Arc[] arcs_9_27 = {
        new Arc(26, 7),
    };

    public static Arc[] arcs_9_28 = {
        new Arc(26, 3),
    };

    public static Arc[] arcs_9_29 = {
        new Arc(34, 26),
        new Arc(30, 19),
    };

    public static Arc[] arcs_9_30 = {
        new Arc(1, -1),
        new Arc(32, 18),
        new Arc(30, 11),
    };

    public static Arc[] arcs_9_31 = {
        new Arc(26, 7),
    };

    public static Arc[] arcs_9_32 = {
        new Arc(30, 10),
    };

    public static Arc[] arcs_9_33 = {
        new Arc(34, 15),
        new Arc(30, 2),
    };

    public static State[] states_9 = {
        new State(3, GramInit.arcs_9_0),
        new State(1, GramInit.arcs_9_1),
        new State(3, GramInit.arcs_9_2),
        new State(2, GramInit.arcs_9_3),
        new State(4, GramInit.arcs_9_4),
        new State(3, GramInit.arcs_9_5),
        new State(3, GramInit.arcs_9_6),
        new State(2, GramInit.arcs_9_7),
        new State(3, GramInit.arcs_9_8),
        new State(1, GramInit.arcs_9_9),
        new State(1, GramInit.arcs_9_10),
        new State(2, GramInit.arcs_9_11),
        new State(3, GramInit.arcs_9_12),
        new State(1, GramInit.arcs_9_13),
        new State(2, GramInit.arcs_9_14),
        new State(1, GramInit.arcs_9_15),
        new State(1, GramInit.arcs_9_16),
        new State(4, GramInit.arcs_9_17),
        new State(2, GramInit.arcs_9_18),
        new State(3, GramInit.arcs_9_19),
        new State(1, GramInit.arcs_9_20),
        new State(2, GramInit.arcs_9_21),
        new State(2, GramInit.arcs_9_22),
        new State(3, GramInit.arcs_9_23),
        new State(3, GramInit.arcs_9_24),
        new State(2, GramInit.arcs_9_25),
        new State(1, GramInit.arcs_9_26),
        new State(1, GramInit.arcs_9_27),
        new State(1, GramInit.arcs_9_28),
        new State(2, GramInit.arcs_9_29),
        new State(3, GramInit.arcs_9_30),
        new State(1, GramInit.arcs_9_31),
        new State(1, GramInit.arcs_9_32),
        new State(2, GramInit.arcs_9_33),
    };

    public static Arc[] arcs_10_0 = {
        new Arc(23, 3),
    };

    public static Arc[] arcs_10_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_10_2 = {
        new Arc(26, 1),
    };

    public static Arc[] arcs_10_3 = {
        new Arc(1, -1),
        new Arc(27, 2),
    };

    public static State[] states_10 = {
        new State(1, GramInit.arcs_10_0),
        new State(1, GramInit.arcs_10_1),
        new State(1, GramInit.arcs_10_2),
        new State(2, GramInit.arcs_10_3),
    };

    public static Arc[] arcs_11_0 = {
        new Arc(34, 4),
        new Arc(33, 6),
        new Arc(36, 12),
    };

    public static Arc[] arcs_11_1 = {
        new Arc(1, -1),
        new Arc(31, 18),
        new Arc(32, 24),
    };

    public static Arc[] arcs_11_2 = {
        new Arc(36, 9),
    };

    public static Arc[] arcs_11_3 = {
        new Arc(26, 19),
    };

    public static Arc[] arcs_11_4 = {
        new Arc(36, 9),
    };

    public static Arc[] arcs_11_5 = {
        new Arc(26, 23),
    };

    public static Arc[] arcs_11_6 = {
        new Arc(1, -1),
        new Arc(32, 29),
        new Arc(36, 33),
    };

    public static Arc[] arcs_11_7 = {
        new Arc(1, -1),
        new Arc(31, 3),
        new Arc(32, 30),
    };

    public static Arc[] arcs_11_8 = {
        new Arc(1, -1),
        new Arc(31, 25),
        new Arc(32, 30),
    };

    public static Arc[] arcs_11_9 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_11_10 = {
        new Arc(1, -1),
        new Arc(32, 22),
        new Arc(36, 11),
    };

    public static Arc[] arcs_11_11 = {
        new Arc(1, -1),
        new Arc(32, 22),
    };

    public static Arc[] arcs_11_12 = {
        new Arc(1, -1),
        new Arc(31, 21),
        new Arc(32, 28),
    };

    public static Arc[] arcs_11_13 = {
        new Arc(34, 16),
        new Arc(36, 14),
    };

    public static Arc[] arcs_11_14 = {
        new Arc(1, -1),
        new Arc(31, 5),
        new Arc(32, 13),
    };

    public static Arc[] arcs_11_15 = {
        new Arc(1, -1),
        new Arc(31, 27),
        new Arc(32, 24),
    };

    public static Arc[] arcs_11_16 = {
        new Arc(36, 9),
    };

    public static Arc[] arcs_11_17 = {
        new Arc(36, 9),
    };

    public static Arc[] arcs_11_18 = {
        new Arc(26, 32),
    };

    public static Arc[] arcs_11_19 = {
        new Arc(1, -1),
        new Arc(32, 30),
    };

    public static Arc[] arcs_11_20 = {
        new Arc(1, -1),
        new Arc(32, 28),
    };

    public static Arc[] arcs_11_21 = {
        new Arc(26, 20),
    };

    public static Arc[] arcs_11_22 = {
        new Arc(34, 16),
        new Arc(36, 31),
    };

    public static Arc[] arcs_11_23 = {
        new Arc(1, -1),
        new Arc(32, 13),
    };

    public static Arc[] arcs_11_24 = {
        new Arc(34, 2),
        new Arc(36, 15),
    };

    public static Arc[] arcs_11_25 = {
        new Arc(26, 19),
    };

    public static Arc[] arcs_11_26 = {
        new Arc(26, 23),
    };

    public static Arc[] arcs_11_27 = {
        new Arc(26, 32),
    };

    public static Arc[] arcs_11_28 = {
        new Arc(1, -1),
        new Arc(34, 17),
        new Arc(33, 10),
        new Arc(36, 7),
    };

    public static Arc[] arcs_11_29 = {
        new Arc(34, 2),
        new Arc(36, 1),
    };

    public static Arc[] arcs_11_30 = {
        new Arc(1, -1),
        new Arc(34, 17),
        new Arc(33, 10),
        new Arc(36, 8),
    };

    public static Arc[] arcs_11_31 = {
        new Arc(1, -1),
        new Arc(31, 26),
        new Arc(32, 13),
    };

    public static Arc[] arcs_11_32 = {
        new Arc(1, -1),
        new Arc(32, 24),
    };

    public static Arc[] arcs_11_33 = {
        new Arc(1, -1),
        new Arc(32, 29),
    };

    public static State[] states_11 = {
        new State(3, GramInit.arcs_11_0),
        new State(3, GramInit.arcs_11_1),
        new State(1, GramInit.arcs_11_2),
        new State(1, GramInit.arcs_11_3),
        new State(1, GramInit.arcs_11_4),
        new State(1, GramInit.arcs_11_5),
        new State(3, GramInit.arcs_11_6),
        new State(3, GramInit.arcs_11_7),
        new State(3, GramInit.arcs_11_8),
        new State(1, GramInit.arcs_11_9),
        new State(3, GramInit.arcs_11_10),
        new State(2, GramInit.arcs_11_11),
        new State(3, GramInit.arcs_11_12),
        new State(2, GramInit.arcs_11_13),
        new State(3, GramInit.arcs_11_14),
        new State(3, GramInit.arcs_11_15),
        new State(1, GramInit.arcs_11_16),
        new State(1, GramInit.arcs_11_17),
        new State(1, GramInit.arcs_11_18),
        new State(2, GramInit.arcs_11_19),
        new State(2, GramInit.arcs_11_20),
        new State(1, GramInit.arcs_11_21),
        new State(2, GramInit.arcs_11_22),
        new State(2, GramInit.arcs_11_23),
        new State(2, GramInit.arcs_11_24),
        new State(1, GramInit.arcs_11_25),
        new State(1, GramInit.arcs_11_26),
        new State(1, GramInit.arcs_11_27),
        new State(4, GramInit.arcs_11_28),
        new State(2, GramInit.arcs_11_29),
        new State(4, GramInit.arcs_11_30),
        new State(3, GramInit.arcs_11_31),
        new State(2, GramInit.arcs_11_32),
        new State(2, GramInit.arcs_11_33),
    };

    public static Arc[] arcs_12_0 = {
        new Arc(23, 1),
    };

    public static Arc[] arcs_12_1 = {
        new Arc(1, -1),
    };

    public static State[] states_12 = {
        new State(1, GramInit.arcs_12_0),
        new State(1, GramInit.arcs_12_1),
    };

    public static Arc[] arcs_13_0 = {
        new Arc(4, 1),
        new Arc(3, 1),
    };

    public static Arc[] arcs_13_1 = {
        new Arc(1, -1),
    };

    public static State[] states_13 = {
        new State(2, GramInit.arcs_13_0),
        new State(1, GramInit.arcs_13_1),
    };

    public static Arc[] arcs_14_0 = {
        new Arc(37, 1),
    };

    public static Arc[] arcs_14_1 = {
        new Arc(2, 3),
        new Arc(38, 4),
    };

    public static Arc[] arcs_14_2 = {
        new Arc(2, 3),
        new Arc(38, 5),
    };

    public static Arc[] arcs_14_3 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_14_4 = {
        new Arc(37, 2),
        new Arc(2, 3),
    };

    public static Arc[] arcs_14_5 = {
        new Arc(37, 2),
        new Arc(2, 3),
    };

    public static State[] states_14 = {
        new State(1, GramInit.arcs_14_0),
        new State(2, GramInit.arcs_14_1),
        new State(2, GramInit.arcs_14_2),
        new State(1, GramInit.arcs_14_3),
        new State(2, GramInit.arcs_14_4),
        new State(2, GramInit.arcs_14_5),
    };

    public static Arc[] arcs_15_0 = {
        new Arc(44, 1),
        new Arc(46, 1),
        new Arc(45, 1),
        new Arc(40, 1),
        new Arc(42, 1),
        new Arc(41, 1),
        new Arc(39, 1),
        new Arc(43, 1),
    };

    public static Arc[] arcs_15_1 = {
        new Arc(1, -1),
    };

    public static State[] states_15 = {
        new State(8, GramInit.arcs_15_0),
        new State(1, GramInit.arcs_15_1),
    };

    public static Arc[] arcs_16_0 = {
        new Arc(47, 4),
    };

    public static Arc[] arcs_16_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_16_2 = {
        new Arc(1, -1),
        new Arc(31, 5),
    };

    public static Arc[] arcs_16_3 = {
        new Arc(49, 2),
        new Arc(47, 2),
    };

    public static Arc[] arcs_16_4 = {
        new Arc(1, -1),
        new Arc(31, 3),
        new Arc(48, 6),
    };

    public static Arc[] arcs_16_5 = {
        new Arc(49, 2),
        new Arc(47, 2),
    };

    public static Arc[] arcs_16_6 = {
        new Arc(49, 1),
        new Arc(9, 1),
    };

    public static State[] states_16 = {
        new State(1, GramInit.arcs_16_0),
        new State(1, GramInit.arcs_16_1),
        new State(2, GramInit.arcs_16_2),
        new State(2, GramInit.arcs_16_3),
        new State(3, GramInit.arcs_16_4),
        new State(2, GramInit.arcs_16_5),
        new State(2, GramInit.arcs_16_6),
    };

    public static Arc[] arcs_17_0 = {
        new Arc(26, 3),
        new Arc(50, 3),
    };

    public static Arc[] arcs_17_1 = {
        new Arc(1, -1),
        new Arc(26, 4),
        new Arc(50, 4),
    };

    public static Arc[] arcs_17_2 = {
        new Arc(1, -1),
        new Arc(26, 4),
        new Arc(50, 4),
    };

    public static Arc[] arcs_17_3 = {
        new Arc(1, -1),
        new Arc(32, 1),
    };

    public static Arc[] arcs_17_4 = {
        new Arc(1, -1),
        new Arc(32, 2),
    };

    public static State[] states_17 = {
        new State(2, GramInit.arcs_17_0),
        new State(3, GramInit.arcs_17_1),
        new State(3, GramInit.arcs_17_2),
        new State(2, GramInit.arcs_17_3),
        new State(2, GramInit.arcs_17_4),
    };

    public static Arc[] arcs_18_0 = {
        new Arc(56, 1),
        new Arc(61, 1),
        new Arc(62, 1),
        new Arc(59, 1),
        new Arc(55, 1),
        new Arc(54, 1),
        new Arc(57, 1),
        new Arc(60, 1),
        new Arc(52, 1),
        new Arc(51, 1),
        new Arc(58, 1),
        new Arc(53, 1),
        new Arc(63, 1),
    };

    public static Arc[] arcs_18_1 = {
        new Arc(1, -1),
    };

    public static State[] states_18 = {
        new State(13, GramInit.arcs_18_0),
        new State(1, GramInit.arcs_18_1),
    };

    public static Arc[] arcs_19_0 = {
        new Arc(64, 2),
    };

    public static Arc[] arcs_19_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_19_2 = {
        new Arc(65, 1),
    };

    public static State[] states_19 = {
        new State(1, GramInit.arcs_19_0),
        new State(1, GramInit.arcs_19_1),
        new State(1, GramInit.arcs_19_2),
    };

    public static Arc[] arcs_20_0 = {
        new Arc(66, 1),
    };

    public static Arc[] arcs_20_1 = {
        new Arc(1, -1),
    };

    public static State[] states_20 = {
        new State(1, GramInit.arcs_20_0),
        new State(1, GramInit.arcs_20_1),
    };

    public static Arc[] arcs_21_0 = {
        new Arc(69, 1),
        new Arc(67, 1),
        new Arc(71, 1),
        new Arc(68, 1),
        new Arc(70, 1),
    };

    public static Arc[] arcs_21_1 = {
        new Arc(1, -1),
    };

    public static State[] states_21 = {
        new State(5, GramInit.arcs_21_0),
        new State(1, GramInit.arcs_21_1),
    };

    public static Arc[] arcs_22_0 = {
        new Arc(72, 1),
    };

    public static Arc[] arcs_22_1 = {
        new Arc(1, -1),
    };

    public static State[] states_22 = {
        new State(1, GramInit.arcs_22_0),
        new State(1, GramInit.arcs_22_1),
    };

    public static Arc[] arcs_23_0 = {
        new Arc(73, 1),
    };

    public static Arc[] arcs_23_1 = {
        new Arc(1, -1),
    };

    public static State[] states_23 = {
        new State(1, GramInit.arcs_23_0),
        new State(1, GramInit.arcs_23_1),
    };

    public static Arc[] arcs_24_0 = {
        new Arc(74, 2),
    };

    public static Arc[] arcs_24_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_24_2 = {
        new Arc(1, -1),
        new Arc(9, 1),
    };

    public static State[] states_24 = {
        new State(1, GramInit.arcs_24_0),
        new State(1, GramInit.arcs_24_1),
        new State(2, GramInit.arcs_24_2),
    };

    public static Arc[] arcs_25_0 = {
        new Arc(49, 1),
    };

    public static Arc[] arcs_25_1 = {
        new Arc(1, -1),
    };

    public static State[] states_25 = {
        new State(1, GramInit.arcs_25_0),
        new State(1, GramInit.arcs_25_1),
    };

    public static Arc[] arcs_26_0 = {
        new Arc(75, 4),
    };

    public static Arc[] arcs_26_1 = {
        new Arc(26, 2),
    };

    public static Arc[] arcs_26_2 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_26_3 = {
        new Arc(1, -1),
        new Arc(76, 1),
    };

    public static Arc[] arcs_26_4 = {
        new Arc(1, -1),
        new Arc(26, 3),
    };

    public static State[] states_26 = {
        new State(1, GramInit.arcs_26_0),
        new State(1, GramInit.arcs_26_1),
        new State(1, GramInit.arcs_26_2),
        new State(2, GramInit.arcs_26_3),
        new State(2, GramInit.arcs_26_4),
    };

    public static Arc[] arcs_27_0 = {
        new Arc(77, 1),
        new Arc(78, 1),
    };

    public static Arc[] arcs_27_1 = {
        new Arc(1, -1),
    };

    public static State[] states_27 = {
        new State(2, GramInit.arcs_27_0),
        new State(1, GramInit.arcs_27_1),
    };

    public static Arc[] arcs_28_0 = {
        new Arc(79, 1),
    };

    public static Arc[] arcs_28_1 = {
        new Arc(80, 2),
    };

    public static Arc[] arcs_28_2 = {
        new Arc(1, -1),
    };

    public static State[] states_28 = {
        new State(1, GramInit.arcs_28_0),
        new State(1, GramInit.arcs_28_1),
        new State(1, GramInit.arcs_28_2),
    };

    public static Arc[] arcs_29_0 = {
        new Arc(76, 2),
    };

    public static Arc[] arcs_29_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_29_2 = {
        new Arc(12, 5),
        new Arc(81, 3),
        new Arc(82, 3),
    };

    public static Arc[] arcs_29_3 = {
        new Arc(12, 5),
        new Arc(81, 3),
        new Arc(79, 7),
        new Arc(82, 3),
    };

    public static Arc[] arcs_29_4 = {
        new Arc(83, 6),
    };

    public static Arc[] arcs_29_5 = {
        new Arc(79, 7),
    };

    public static Arc[] arcs_29_6 = {
        new Arc(15, 1),
    };

    public static Arc[] arcs_29_7 = {
        new Arc(13, 4),
        new Arc(33, 1),
        new Arc(83, 1),
    };

    public static State[] states_29 = {
        new State(1, GramInit.arcs_29_0),
        new State(1, GramInit.arcs_29_1),
        new State(3, GramInit.arcs_29_2),
        new State(4, GramInit.arcs_29_3),
        new State(1, GramInit.arcs_29_4),
        new State(1, GramInit.arcs_29_5),
        new State(1, GramInit.arcs_29_6),
        new State(3, GramInit.arcs_29_7),
    };

    public static Arc[] arcs_30_0 = {
        new Arc(23, 3),
    };

    public static Arc[] arcs_30_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_30_2 = {
        new Arc(23, 1),
    };

    public static Arc[] arcs_30_3 = {
        new Arc(1, -1),
        new Arc(85, 2),
    };

    public static State[] states_30 = {
        new State(1, GramInit.arcs_30_0),
        new State(1, GramInit.arcs_30_1),
        new State(1, GramInit.arcs_30_2),
        new State(2, GramInit.arcs_30_3),
    };

    public static Arc[] arcs_31_0 = {
        new Arc(12, 2),
    };

    public static Arc[] arcs_31_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_31_2 = {
        new Arc(1, -1),
        new Arc(85, 3),
    };

    public static Arc[] arcs_31_3 = {
        new Arc(23, 1),
    };

    public static State[] states_31 = {
        new State(1, GramInit.arcs_31_0),
        new State(1, GramInit.arcs_31_1),
        new State(2, GramInit.arcs_31_2),
        new State(1, GramInit.arcs_31_3),
    };

    public static Arc[] arcs_32_0 = {
        new Arc(84, 3),
    };

    public static Arc[] arcs_32_1 = {
        new Arc(1, -1),
        new Arc(84, 4),
    };

    public static Arc[] arcs_32_2 = {
        new Arc(1, -1),
        new Arc(84, 4),
    };

    public static Arc[] arcs_32_3 = {
        new Arc(1, -1),
        new Arc(32, 2),
    };

    public static Arc[] arcs_32_4 = {
        new Arc(1, -1),
        new Arc(32, 1),
    };

    public static State[] states_32 = {
        new State(1, GramInit.arcs_32_0),
        new State(2, GramInit.arcs_32_1),
        new State(2, GramInit.arcs_32_2),
        new State(2, GramInit.arcs_32_3),
        new State(2, GramInit.arcs_32_4),
    };

    public static Arc[] arcs_33_0 = {
        new Arc(86, 4),
    };

    public static Arc[] arcs_33_1 = {
        new Arc(86, 2),
    };

    public static Arc[] arcs_33_2 = {
        new Arc(1, -1),
        new Arc(32, 3),
    };

    public static Arc[] arcs_33_3 = {
        new Arc(86, 2),
    };

    public static Arc[] arcs_33_4 = {
        new Arc(1, -1),
        new Arc(32, 1),
    };

    public static State[] states_33 = {
        new State(1, GramInit.arcs_33_0),
        new State(1, GramInit.arcs_33_1),
        new State(2, GramInit.arcs_33_2),
        new State(1, GramInit.arcs_33_3),
        new State(2, GramInit.arcs_33_4),
    };

    public static Arc[] arcs_34_0 = {
        new Arc(23, 1),
    };

    public static Arc[] arcs_34_1 = {
        new Arc(1, -1),
        new Arc(81, 3),
    };

    public static Arc[] arcs_34_2 = {
        new Arc(1, -1),
        new Arc(81, 4),
    };

    public static Arc[] arcs_34_3 = {
        new Arc(23, 2),
    };

    public static Arc[] arcs_34_4 = {
        new Arc(23, 2),
    };

    public static State[] states_34 = {
        new State(1, GramInit.arcs_34_0),
        new State(2, GramInit.arcs_34_1),
        new State(2, GramInit.arcs_34_2),
        new State(1, GramInit.arcs_34_3),
        new State(1, GramInit.arcs_34_4),
    };

    public static Arc[] arcs_35_0 = {
        new Arc(87, 1),
    };

    public static Arc[] arcs_35_1 = {
        new Arc(23, 2),
    };

    public static Arc[] arcs_35_2 = {
        new Arc(1, -1),
        new Arc(32, 4),
    };

    public static Arc[] arcs_35_3 = {
        new Arc(1, -1),
        new Arc(32, 5),
    };

    public static Arc[] arcs_35_4 = {
        new Arc(23, 3),
    };

    public static Arc[] arcs_35_5 = {
        new Arc(23, 3),
    };

    public static State[] states_35 = {
        new State(1, GramInit.arcs_35_0),
        new State(1, GramInit.arcs_35_1),
        new State(2, GramInit.arcs_35_2),
        new State(2, GramInit.arcs_35_3),
        new State(1, GramInit.arcs_35_4),
        new State(1, GramInit.arcs_35_5),
    };

    public static Arc[] arcs_36_0 = {
        new Arc(88, 4),
    };

    public static Arc[] arcs_36_1 = {
        new Arc(1, -1),
        new Arc(32, 2),
    };

    public static Arc[] arcs_36_2 = {
        new Arc(23, 1),
    };

    public static Arc[] arcs_36_3 = {
        new Arc(23, 1),
    };

    public static Arc[] arcs_36_4 = {
        new Arc(23, 5),
    };

    public static Arc[] arcs_36_5 = {
        new Arc(1, -1),
        new Arc(32, 3),
    };

    public static State[] states_36 = {
        new State(1, GramInit.arcs_36_0),
        new State(2, GramInit.arcs_36_1),
        new State(1, GramInit.arcs_36_2),
        new State(1, GramInit.arcs_36_3),
        new State(1, GramInit.arcs_36_4),
        new State(2, GramInit.arcs_36_5),
    };

    public static Arc[] arcs_37_0 = {
        new Arc(89, 1),
    };

    public static Arc[] arcs_37_1 = {
        new Arc(26, 4),
    };

    public static Arc[] arcs_37_2 = {
        new Arc(26, 3),
    };

    public static Arc[] arcs_37_3 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_37_4 = {
        new Arc(1, -1),
        new Arc(32, 2),
    };

    public static State[] states_37 = {
        new State(1, GramInit.arcs_37_0),
        new State(1, GramInit.arcs_37_1),
        new State(1, GramInit.arcs_37_2),
        new State(1, GramInit.arcs_37_3),
        new State(2, GramInit.arcs_37_4),
    };

    public static Arc[] arcs_38_0 = {
        new Arc(92, 1),
        new Arc(93, 1),
        new Arc(95, 1),
        new Arc(18, 1),
        new Arc(91, 1),
        new Arc(19, 1),
        new Arc(94, 1),
        new Arc(90, 1),
        new Arc(17, 1),
    };

    public static Arc[] arcs_38_1 = {
        new Arc(1, -1),
    };

    public static State[] states_38 = {
        new State(9, GramInit.arcs_38_0),
        new State(1, GramInit.arcs_38_1),
    };

    public static Arc[] arcs_39_0 = {
        new Arc(21, 1),
    };

    public static Arc[] arcs_39_1 = {
        new Arc(92, 2),
        new Arc(19, 2),
        new Arc(94, 2),
    };

    public static Arc[] arcs_39_2 = {
        new Arc(1, -1),
    };

    public static State[] states_39 = {
        new State(1, GramInit.arcs_39_0),
        new State(3, GramInit.arcs_39_1),
        new State(1, GramInit.arcs_39_2),
    };

    public static Arc[] arcs_40_0 = {
        new Arc(96, 12),
    };

    public static Arc[] arcs_40_1 = {
        new Arc(28, 13),
    };

    public static Arc[] arcs_40_2 = {
        new Arc(27, 5),
    };

    public static Arc[] arcs_40_3 = {
        new Arc(28, 13),
    };

    public static Arc[] arcs_40_4 = {
        new Arc(27, 3),
    };

    public static Arc[] arcs_40_5 = {
        new Arc(28, 10),
    };

    public static Arc[] arcs_40_6 = {
        new Arc(28, 9),
    };

    public static Arc[] arcs_40_7 = {
        new Arc(26, 4),
    };

    public static Arc[] arcs_40_8 = {
        new Arc(26, 11),
    };

    public static Arc[] arcs_40_9 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_40_10 = {
        new Arc(1, -1),
        new Arc(98, 14),
        new Arc(97, 7),
    };

    public static Arc[] arcs_40_11 = {
        new Arc(27, 1),
    };

    public static Arc[] arcs_40_12 = {
        new Arc(26, 2),
    };

    public static Arc[] arcs_40_13 = {
        new Arc(1, -1),
        new Arc(98, 14),
        new Arc(97, 8),
    };

    public static Arc[] arcs_40_14 = {
        new Arc(27, 6),
    };

    public static State[] states_40 = {
        new State(1, GramInit.arcs_40_0),
        new State(1, GramInit.arcs_40_1),
        new State(1, GramInit.arcs_40_2),
        new State(1, GramInit.arcs_40_3),
        new State(1, GramInit.arcs_40_4),
        new State(1, GramInit.arcs_40_5),
        new State(1, GramInit.arcs_40_6),
        new State(1, GramInit.arcs_40_7),
        new State(1, GramInit.arcs_40_8),
        new State(1, GramInit.arcs_40_9),
        new State(3, GramInit.arcs_40_10),
        new State(1, GramInit.arcs_40_11),
        new State(1, GramInit.arcs_40_12),
        new State(3, GramInit.arcs_40_13),
        new State(1, GramInit.arcs_40_14),
    };

    public static Arc[] arcs_41_0 = {
        new Arc(99, 6),
    };

    public static Arc[] arcs_41_1 = {
        new Arc(28, 5),
    };

    public static Arc[] arcs_41_2 = {
        new Arc(27, 3),
    };

    public static Arc[] arcs_41_3 = {
        new Arc(28, 4),
    };

    public static Arc[] arcs_41_4 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_41_5 = {
        new Arc(1, -1),
        new Arc(98, 2),
    };

    public static Arc[] arcs_41_6 = {
        new Arc(26, 7),
    };

    public static Arc[] arcs_41_7 = {
        new Arc(27, 1),
    };

    public static State[] states_41 = {
        new State(1, GramInit.arcs_41_0),
        new State(1, GramInit.arcs_41_1),
        new State(1, GramInit.arcs_41_2),
        new State(1, GramInit.arcs_41_3),
        new State(1, GramInit.arcs_41_4),
        new State(2, GramInit.arcs_41_5),
        new State(1, GramInit.arcs_41_6),
        new State(1, GramInit.arcs_41_7),
    };

    public static Arc[] arcs_42_0 = {
        new Arc(100, 7),
    };

    public static Arc[] arcs_42_1 = {
        new Arc(101, 3),
    };

    public static Arc[] arcs_42_2 = {
        new Arc(28, 8),
    };

    public static Arc[] arcs_42_3 = {
        new Arc(9, 4),
    };

    public static Arc[] arcs_42_4 = {
        new Arc(27, 2),
    };

    public static Arc[] arcs_42_5 = {
        new Arc(27, 6),
    };

    public static Arc[] arcs_42_6 = {
        new Arc(28, 9),
    };

    public static Arc[] arcs_42_7 = {
        new Arc(65, 1),
    };

    public static Arc[] arcs_42_8 = {
        new Arc(1, -1),
        new Arc(98, 5),
    };

    public static Arc[] arcs_42_9 = {
        new Arc(1, -1),
    };

    public static State[] states_42 = {
        new State(1, GramInit.arcs_42_0),
        new State(1, GramInit.arcs_42_1),
        new State(1, GramInit.arcs_42_2),
        new State(1, GramInit.arcs_42_3),
        new State(1, GramInit.arcs_42_4),
        new State(1, GramInit.arcs_42_5),
        new State(1, GramInit.arcs_42_6),
        new State(1, GramInit.arcs_42_7),
        new State(2, GramInit.arcs_42_8),
        new State(1, GramInit.arcs_42_9),
    };

    public static Arc[] arcs_43_0 = {
        new Arc(102, 11),
    };

    public static Arc[] arcs_43_1 = {
        new Arc(104, 16),
        new Arc(103, 2),
    };

    public static Arc[] arcs_43_2 = {
        new Arc(27, 14),
    };

    public static Arc[] arcs_43_3 = {
        new Arc(28, 9),
    };

    public static Arc[] arcs_43_4 = {
        new Arc(28, 5),
    };

    public static Arc[] arcs_43_5 = {
        new Arc(1, -1),
        new Arc(104, 10),
    };

    public static Arc[] arcs_43_6 = {
        new Arc(28, 1),
    };

    public static Arc[] arcs_43_7 = {
        new Arc(27, 4),
    };

    public static Arc[] arcs_43_8 = {
        new Arc(28, 9),
    };

    public static Arc[] arcs_43_9 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_43_10 = {
        new Arc(27, 3),
    };

    public static Arc[] arcs_43_11 = {
        new Arc(27, 6),
    };

    public static Arc[] arcs_43_12 = {
        new Arc(1, -1),
        new Arc(98, 7),
        new Arc(104, 10),
        new Arc(103, 15),
    };

    public static Arc[] arcs_43_13 = {
        new Arc(28, 12),
    };

    public static Arc[] arcs_43_14 = {
        new Arc(28, 12),
    };

    public static Arc[] arcs_43_15 = {
        new Arc(27, 13),
    };

    public static Arc[] arcs_43_16 = {
        new Arc(27, 8),
    };

    public static State[] states_43 = {
        new State(1, GramInit.arcs_43_0),
        new State(2, GramInit.arcs_43_1),
        new State(1, GramInit.arcs_43_2),
        new State(1, GramInit.arcs_43_3),
        new State(1, GramInit.arcs_43_4),
        new State(2, GramInit.arcs_43_5),
        new State(1, GramInit.arcs_43_6),
        new State(1, GramInit.arcs_43_7),
        new State(1, GramInit.arcs_43_8),
        new State(1, GramInit.arcs_43_9),
        new State(1, GramInit.arcs_43_10),
        new State(1, GramInit.arcs_43_11),
        new State(4, GramInit.arcs_43_12),
        new State(1, GramInit.arcs_43_13),
        new State(1, GramInit.arcs_43_14),
        new State(1, GramInit.arcs_43_15),
        new State(1, GramInit.arcs_43_16),
    };

    public static Arc[] arcs_44_0 = {
        new Arc(105, 6),
    };

    public static Arc[] arcs_44_1 = {
        new Arc(27, 4),
        new Arc(32, 3),
    };

    public static Arc[] arcs_44_2 = {
        new Arc(27, 4),
        new Arc(32, 7),
    };

    public static Arc[] arcs_44_3 = {
        new Arc(106, 1),
    };

    public static Arc[] arcs_44_4 = {
        new Arc(28, 5),
    };

    public static Arc[] arcs_44_5 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_44_6 = {
        new Arc(106, 2),
    };

    public static Arc[] arcs_44_7 = {
        new Arc(106, 1),
    };

    public static State[] states_44 = {
        new State(1, GramInit.arcs_44_0),
        new State(2, GramInit.arcs_44_1),
        new State(2, GramInit.arcs_44_2),
        new State(1, GramInit.arcs_44_3),
        new State(1, GramInit.arcs_44_4),
        new State(1, GramInit.arcs_44_5),
        new State(1, GramInit.arcs_44_6),
        new State(1, GramInit.arcs_44_7),
    };

    public static Arc[] arcs_45_0 = {
        new Arc(26, 1),
    };

    public static Arc[] arcs_45_1 = {
        new Arc(1, -1),
        new Arc(85, 2),
    };

    public static Arc[] arcs_45_2 = {
        new Arc(107, 3),
    };

    public static Arc[] arcs_45_3 = {
        new Arc(1, -1),
    };

    public static State[] states_45 = {
        new State(1, GramInit.arcs_45_0),
        new State(2, GramInit.arcs_45_1),
        new State(1, GramInit.arcs_45_2),
        new State(1, GramInit.arcs_45_3),
    };

    public static Arc[] arcs_46_0 = {
        new Arc(108, 3),
    };

    public static Arc[] arcs_46_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_46_2 = {
        new Arc(23, 1),
    };

    public static Arc[] arcs_46_3 = {
        new Arc(1, -1),
        new Arc(26, 4),
    };

    public static Arc[] arcs_46_4 = {
        new Arc(1, -1),
        new Arc(85, 2),
    };

    public static State[] states_46 = {
        new State(1, GramInit.arcs_46_0),
        new State(1, GramInit.arcs_46_1),
        new State(1, GramInit.arcs_46_2),
        new State(2, GramInit.arcs_46_3),
        new State(2, GramInit.arcs_46_4),
    };

    public static Arc[] arcs_47_0 = {
        new Arc(2, 1),
        new Arc(3, 3),
    };

    public static Arc[] arcs_47_1 = {
        new Arc(109, 4),
    };

    public static Arc[] arcs_47_2 = {
        new Arc(110, 3),
        new Arc(6, 2),
    };

    public static Arc[] arcs_47_3 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_47_4 = {
        new Arc(6, 2),
    };

    public static State[] states_47 = {
        new State(2, GramInit.arcs_47_0),
        new State(1, GramInit.arcs_47_1),
        new State(2, GramInit.arcs_47_2),
        new State(1, GramInit.arcs_47_3),
        new State(1, GramInit.arcs_47_4),
    };

    public static Arc[] arcs_48_0 = {
        new Arc(111, 5),
        new Arc(112, 1),
    };

    public static Arc[] arcs_48_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_48_2 = {
        new Arc(111, 4),
    };

    public static Arc[] arcs_48_3 = {
        new Arc(26, 1),
    };

    public static Arc[] arcs_48_4 = {
        new Arc(98, 3),
    };

    public static Arc[] arcs_48_5 = {
        new Arc(1, -1),
        new Arc(96, 2),
    };

    public static State[] states_48 = {
        new State(2, GramInit.arcs_48_0),
        new State(1, GramInit.arcs_48_1),
        new State(1, GramInit.arcs_48_2),
        new State(1, GramInit.arcs_48_3),
        new State(1, GramInit.arcs_48_4),
        new State(2, GramInit.arcs_48_5),
    };

    public static Arc[] arcs_49_0 = {
        new Arc(114, 1),
        new Arc(111, 1),
    };

    public static Arc[] arcs_49_1 = {
        new Arc(1, -1),
    };

    public static State[] states_49 = {
        new State(2, GramInit.arcs_49_0),
        new State(1, GramInit.arcs_49_1),
    };

    public static Arc[] arcs_50_0 = {
        new Arc(115, 1),
    };

    public static Arc[] arcs_50_1 = {
        new Arc(27, 2),
        new Arc(35, 4),
    };

    public static Arc[] arcs_50_2 = {
        new Arc(26, 3),
    };

    public static Arc[] arcs_50_3 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_50_4 = {
        new Arc(27, 2),
    };

    public static State[] states_50 = {
        new State(1, GramInit.arcs_50_0),
        new State(2, GramInit.arcs_50_1),
        new State(1, GramInit.arcs_50_2),
        new State(1, GramInit.arcs_50_3),
        new State(1, GramInit.arcs_50_4),
    };

    public static Arc[] arcs_51_0 = {
        new Arc(115, 3),
    };

    public static Arc[] arcs_51_1 = {
        new Arc(27, 2),
    };

    public static Arc[] arcs_51_2 = {
        new Arc(113, 4),
    };

    public static Arc[] arcs_51_3 = {
        new Arc(27, 2),
        new Arc(35, 1),
    };

    public static Arc[] arcs_51_4 = {
        new Arc(1, -1),
    };

    public static State[] states_51 = {
        new State(1, GramInit.arcs_51_0),
        new State(1, GramInit.arcs_51_1),
        new State(1, GramInit.arcs_51_2),
        new State(2, GramInit.arcs_51_3),
        new State(1, GramInit.arcs_51_4),
    };

    public static Arc[] arcs_52_0 = {
        new Arc(116, 1),
    };

    public static Arc[] arcs_52_1 = {
        new Arc(1, -1),
        new Arc(117, 4),
    };

    public static Arc[] arcs_52_2 = {
        new Arc(1, -1),
        new Arc(117, 3),
    };

    public static Arc[] arcs_52_3 = {
        new Arc(116, 2),
    };

    public static Arc[] arcs_52_4 = {
        new Arc(116, 2),
    };

    public static State[] states_52 = {
        new State(1, GramInit.arcs_52_0),
        new State(2, GramInit.arcs_52_1),
        new State(2, GramInit.arcs_52_2),
        new State(1, GramInit.arcs_52_3),
        new State(1, GramInit.arcs_52_4),
    };

    public static Arc[] arcs_53_0 = {
        new Arc(118, 1),
    };

    public static Arc[] arcs_53_1 = {
        new Arc(1, -1),
        new Arc(119, 2),
    };

    public static Arc[] arcs_53_2 = {
        new Arc(118, 4),
    };

    public static Arc[] arcs_53_3 = {
        new Arc(118, 4),
    };

    public static Arc[] arcs_53_4 = {
        new Arc(1, -1),
        new Arc(119, 3),
    };

    public static State[] states_53 = {
        new State(1, GramInit.arcs_53_0),
        new State(2, GramInit.arcs_53_1),
        new State(1, GramInit.arcs_53_2),
        new State(1, GramInit.arcs_53_3),
        new State(2, GramInit.arcs_53_4),
    };

    public static Arc[] arcs_54_0 = {
        new Arc(120, 1),
        new Arc(121, 2),
    };

    public static Arc[] arcs_54_1 = {
        new Arc(118, 2),
    };

    public static Arc[] arcs_54_2 = {
        new Arc(1, -1),
    };

    public static State[] states_54 = {
        new State(2, GramInit.arcs_54_0),
        new State(1, GramInit.arcs_54_1),
        new State(1, GramInit.arcs_54_2),
    };

    public static Arc[] arcs_55_0 = {
        new Arc(107, 3),
    };

    public static Arc[] arcs_55_1 = {
        new Arc(107, 4),
    };

    public static Arc[] arcs_55_2 = {
        new Arc(107, 4),
    };

    public static Arc[] arcs_55_3 = {
        new Arc(1, -1),
        new Arc(122, 2),
    };

    public static Arc[] arcs_55_4 = {
        new Arc(1, -1),
        new Arc(122, 1),
    };

    public static State[] states_55 = {
        new State(1, GramInit.arcs_55_0),
        new State(1, GramInit.arcs_55_1),
        new State(1, GramInit.arcs_55_2),
        new State(2, GramInit.arcs_55_3),
        new State(2, GramInit.arcs_55_4),
    };

    public static Arc[] arcs_56_0 = {
        new Arc(120, 3),
        new Arc(127, 1),
        new Arc(101, 1),
        new Arc(128, 1),
        new Arc(123, 1),
        new Arc(125, 1),
        new Arc(129, 2),
        new Arc(124, 1),
        new Arc(126, 1),
    };

    public static Arc[] arcs_56_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_56_2 = {
        new Arc(1, -1),
        new Arc(120, 1),
    };

    public static Arc[] arcs_56_3 = {
        new Arc(101, 1),
    };

    public static State[] states_56 = {
        new State(9, GramInit.arcs_56_0),
        new State(1, GramInit.arcs_56_1),
        new State(2, GramInit.arcs_56_2),
        new State(1, GramInit.arcs_56_3),
    };

    public static Arc[] arcs_57_0 = {
        new Arc(33, 1),
    };

    public static Arc[] arcs_57_1 = {
        new Arc(107, 2),
    };

    public static Arc[] arcs_57_2 = {
        new Arc(1, -1),
    };

    public static State[] states_57 = {
        new State(1, GramInit.arcs_57_0),
        new State(1, GramInit.arcs_57_1),
        new State(1, GramInit.arcs_57_2),
    };

    public static Arc[] arcs_58_0 = {
        new Arc(130, 4),
    };

    public static Arc[] arcs_58_1 = {
        new Arc(1, -1),
        new Arc(131, 2),
    };

    public static Arc[] arcs_58_2 = {
        new Arc(130, 1),
    };

    public static Arc[] arcs_58_3 = {
        new Arc(130, 1),
    };

    public static Arc[] arcs_58_4 = {
        new Arc(1, -1),
        new Arc(131, 3),
    };

    public static State[] states_58 = {
        new State(1, GramInit.arcs_58_0),
        new State(2, GramInit.arcs_58_1),
        new State(1, GramInit.arcs_58_2),
        new State(1, GramInit.arcs_58_3),
        new State(2, GramInit.arcs_58_4),
    };

    public static Arc[] arcs_59_0 = {
        new Arc(132, 3),
    };

    public static Arc[] arcs_59_1 = {
        new Arc(132, 4),
    };

    public static Arc[] arcs_59_2 = {
        new Arc(132, 4),
    };

    public static Arc[] arcs_59_3 = {
        new Arc(1, -1),
        new Arc(133, 2),
    };

    public static Arc[] arcs_59_4 = {
        new Arc(1, -1),
        new Arc(133, 1),
    };

    public static State[] states_59 = {
        new State(1, GramInit.arcs_59_0),
        new State(1, GramInit.arcs_59_1),
        new State(1, GramInit.arcs_59_2),
        new State(2, GramInit.arcs_59_3),
        new State(2, GramInit.arcs_59_4),
    };

    public static Arc[] arcs_60_0 = {
        new Arc(134, 3),
    };

    public static Arc[] arcs_60_1 = {
        new Arc(134, 2),
    };

    public static Arc[] arcs_60_2 = {
        new Arc(1, -1),
        new Arc(135, 1),
    };

    public static Arc[] arcs_60_3 = {
        new Arc(1, -1),
        new Arc(135, 4),
    };

    public static Arc[] arcs_60_4 = {
        new Arc(134, 2),
    };

    public static State[] states_60 = {
        new State(1, GramInit.arcs_60_0),
        new State(1, GramInit.arcs_60_1),
        new State(2, GramInit.arcs_60_2),
        new State(2, GramInit.arcs_60_3),
        new State(1, GramInit.arcs_60_4),
    };

    public static Arc[] arcs_61_0 = {
        new Arc(136, 4),
    };

    public static Arc[] arcs_61_1 = {
        new Arc(136, 3),
    };

    public static Arc[] arcs_61_2 = {
        new Arc(136, 3),
    };

    public static Arc[] arcs_61_3 = {
        new Arc(1, -1),
        new Arc(138, 1),
        new Arc(137, 1),
    };

    public static Arc[] arcs_61_4 = {
        new Arc(1, -1),
        new Arc(138, 2),
        new Arc(137, 2),
    };

    public static State[] states_61 = {
        new State(1, GramInit.arcs_61_0),
        new State(1, GramInit.arcs_61_1),
        new State(1, GramInit.arcs_61_2),
        new State(3, GramInit.arcs_61_3),
        new State(3, GramInit.arcs_61_4),
    };

    public static Arc[] arcs_62_0 = {
        new Arc(139, 2),
    };

    public static Arc[] arcs_62_1 = {
        new Arc(139, 4),
    };

    public static Arc[] arcs_62_2 = {
        new Arc(1, -1),
        new Arc(141, 1),
        new Arc(140, 1),
    };

    public static Arc[] arcs_62_3 = {
        new Arc(139, 4),
    };

    public static Arc[] arcs_62_4 = {
        new Arc(1, -1),
        new Arc(141, 3),
        new Arc(140, 3),
    };

    public static State[] states_62 = {
        new State(1, GramInit.arcs_62_0),
        new State(1, GramInit.arcs_62_1),
        new State(3, GramInit.arcs_62_2),
        new State(1, GramInit.arcs_62_3),
        new State(3, GramInit.arcs_62_4),
    };

    public static Arc[] arcs_63_0 = {
        new Arc(142, 3),
    };

    public static Arc[] arcs_63_1 = {
        new Arc(142, 4),
    };

    public static Arc[] arcs_63_2 = {
        new Arc(142, 4),
    };

    public static Arc[] arcs_63_3 = {
        new Arc(1, -1),
        new Arc(145, 1),
        new Arc(144, 1),
        new Arc(11, 1),
        new Arc(143, 1),
        new Arc(33, 1),
    };

    public static Arc[] arcs_63_4 = {
        new Arc(1, -1),
        new Arc(145, 2),
        new Arc(144, 2),
        new Arc(11, 2),
        new Arc(143, 2),
        new Arc(33, 2),
    };

    public static State[] states_63 = {
        new State(1, GramInit.arcs_63_0),
        new State(1, GramInit.arcs_63_1),
        new State(1, GramInit.arcs_63_2),
        new State(6, GramInit.arcs_63_3),
        new State(6, GramInit.arcs_63_4),
    };

    public static Arc[] arcs_64_0 = {
        new Arc(147, 2),
        new Arc(146, 1),
        new Arc(141, 1),
        new Arc(140, 1),
    };

    public static Arc[] arcs_64_1 = {
        new Arc(142, 2),
    };

    public static Arc[] arcs_64_2 = {
        new Arc(1, -1),
    };

    public static State[] states_64 = {
        new State(4, GramInit.arcs_64_0),
        new State(1, GramInit.arcs_64_1),
        new State(1, GramInit.arcs_64_2),
    };

    public static Arc[] arcs_65_0 = {
        new Arc(148, 1),
    };

    public static Arc[] arcs_65_1 = {
        new Arc(1, -1),
        new Arc(34, 3),
    };

    public static Arc[] arcs_65_2 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_65_3 = {
        new Arc(142, 2),
    };

    public static State[] states_65 = {
        new State(1, GramInit.arcs_65_0),
        new State(2, GramInit.arcs_65_1),
        new State(1, GramInit.arcs_65_2),
        new State(1, GramInit.arcs_65_3),
    };

    public static Arc[] arcs_66_0 = {
        new Arc(150, 2),
        new Arc(149, 3),
    };

    public static Arc[] arcs_66_1 = {
        new Arc(1, -1),
        new Arc(151, 1),
    };

    public static Arc[] arcs_66_2 = {
        new Arc(1, -1),
        new Arc(151, 1),
    };

    public static Arc[] arcs_66_3 = {
        new Arc(150, 2),
    };

    public static State[] states_66 = {
        new State(2, GramInit.arcs_66_0),
        new State(2, GramInit.arcs_66_1),
        new State(2, GramInit.arcs_66_2),
        new State(1, GramInit.arcs_66_3),
    };

    public static Arc[] arcs_67_0 = {
        new Arc(158, 7),
        new Arc(160, 7),
        new Arc(153, 2),
        new Arc(161, 7),
        new Arc(155, 1),
        new Arc(159, 4),
        new Arc(13, 3),
        new Arc(82, 7),
        new Arc(162, 7),
        new Arc(23, 7),
    };

    public static Arc[] arcs_67_1 = {
        new Arc(157, 7),
        new Arc(156, 6),
    };

    public static Arc[] arcs_67_2 = {
        new Arc(154, 7),
        new Arc(152, 8),
    };

    public static Arc[] arcs_67_3 = {
        new Arc(49, 5),
        new Arc(15, 7),
        new Arc(152, 5),
    };

    public static Arc[] arcs_67_4 = {
        new Arc(1, -1),
        new Arc(159, 4),
    };

    public static Arc[] arcs_67_5 = {
        new Arc(15, 7),
    };

    public static Arc[] arcs_67_6 = {
        new Arc(157, 7),
    };

    public static Arc[] arcs_67_7 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_67_8 = {
        new Arc(154, 7),
    };

    public static State[] states_67 = {
        new State(10, GramInit.arcs_67_0),
        new State(2, GramInit.arcs_67_1),
        new State(2, GramInit.arcs_67_2),
        new State(3, GramInit.arcs_67_3),
        new State(2, GramInit.arcs_67_4),
        new State(1, GramInit.arcs_67_5),
        new State(1, GramInit.arcs_67_6),
        new State(1, GramInit.arcs_67_7),
        new State(1, GramInit.arcs_67_8),
    };

    public static Arc[] arcs_68_0 = {
        new Arc(26, 2),
        new Arc(50, 2),
    };

    public static Arc[] arcs_68_1 = {
        new Arc(1, -1),
        new Arc(32, 4),
    };

    public static Arc[] arcs_68_2 = {
        new Arc(1, -1),
        new Arc(163, 5),
        new Arc(32, 3),
    };

    public static Arc[] arcs_68_3 = {
        new Arc(1, -1),
        new Arc(26, 1),
        new Arc(50, 1),
    };

    public static Arc[] arcs_68_4 = {
        new Arc(1, -1),
        new Arc(26, 1),
        new Arc(50, 1),
    };

    public static Arc[] arcs_68_5 = {
        new Arc(1, -1),
    };

    public static State[] states_68 = {
        new State(2, GramInit.arcs_68_0),
        new State(2, GramInit.arcs_68_1),
        new State(3, GramInit.arcs_68_2),
        new State(3, GramInit.arcs_68_3),
        new State(3, GramInit.arcs_68_4),
        new State(1, GramInit.arcs_68_5),
    };

    public static Arc[] arcs_69_0 = {
        new Arc(153, 5),
        new Arc(81, 4),
        new Arc(13, 1),
    };

    public static Arc[] arcs_69_1 = {
        new Arc(14, 3),
        new Arc(15, 2),
    };

    public static Arc[] arcs_69_2 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_69_3 = {
        new Arc(15, 2),
    };

    public static Arc[] arcs_69_4 = {
        new Arc(23, 2),
    };

    public static Arc[] arcs_69_5 = {
        new Arc(164, 6),
    };

    public static Arc[] arcs_69_6 = {
        new Arc(154, 2),
    };

    public static State[] states_69 = {
        new State(3, GramInit.arcs_69_0),
        new State(2, GramInit.arcs_69_1),
        new State(1, GramInit.arcs_69_2),
        new State(1, GramInit.arcs_69_3),
        new State(1, GramInit.arcs_69_4),
        new State(1, GramInit.arcs_69_5),
        new State(1, GramInit.arcs_69_6),
    };

    public static Arc[] arcs_70_0 = {
        new Arc(165, 2),
    };

    public static Arc[] arcs_70_1 = {
        new Arc(1, -1),
        new Arc(165, 3),
    };

    public static Arc[] arcs_70_2 = {
        new Arc(1, -1),
        new Arc(32, 4),
    };

    public static Arc[] arcs_70_3 = {
        new Arc(1, -1),
        new Arc(32, 1),
    };

    public static Arc[] arcs_70_4 = {
        new Arc(1, -1),
        new Arc(165, 3),
    };

    public static State[] states_70 = {
        new State(1, GramInit.arcs_70_0),
        new State(2, GramInit.arcs_70_1),
        new State(2, GramInit.arcs_70_2),
        new State(2, GramInit.arcs_70_3),
        new State(2, GramInit.arcs_70_4),
    };

    public static Arc[] arcs_71_0 = {
        new Arc(27, 2),
        new Arc(26, 3),
    };

    public static Arc[] arcs_71_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_71_2 = {
        new Arc(1, -1),
        new Arc(26, 4),
        new Arc(166, 1),
    };

    public static Arc[] arcs_71_3 = {
        new Arc(1, -1),
        new Arc(27, 2),
    };

    public static Arc[] arcs_71_4 = {
        new Arc(1, -1),
        new Arc(166, 1),
    };

    public static State[] states_71 = {
        new State(2, GramInit.arcs_71_0),
        new State(1, GramInit.arcs_71_1),
        new State(3, GramInit.arcs_71_2),
        new State(2, GramInit.arcs_71_3),
        new State(2, GramInit.arcs_71_4),
    };

    public static Arc[] arcs_72_0 = {
        new Arc(27, 1),
    };

    public static Arc[] arcs_72_1 = {
        new Arc(1, -1),
        new Arc(26, 2),
    };

    public static Arc[] arcs_72_2 = {
        new Arc(1, -1),
    };

    public static State[] states_72 = {
        new State(1, GramInit.arcs_72_0),
        new State(2, GramInit.arcs_72_1),
        new State(1, GramInit.arcs_72_2),
    };

    public static Arc[] arcs_73_0 = {
        new Arc(107, 3),
        new Arc(50, 3),
    };

    public static Arc[] arcs_73_1 = {
        new Arc(1, -1),
        new Arc(32, 2),
    };

    public static Arc[] arcs_73_2 = {
        new Arc(1, -1),
        new Arc(107, 1),
        new Arc(50, 1),
    };

    public static Arc[] arcs_73_3 = {
        new Arc(1, -1),
        new Arc(32, 4),
    };

    public static Arc[] arcs_73_4 = {
        new Arc(1, -1),
        new Arc(107, 1),
        new Arc(50, 1),
    };

    public static State[] states_73 = {
        new State(2, GramInit.arcs_73_0),
        new State(2, GramInit.arcs_73_1),
        new State(3, GramInit.arcs_73_2),
        new State(2, GramInit.arcs_73_3),
        new State(3, GramInit.arcs_73_4),
    };

    public static Arc[] arcs_74_0 = {
        new Arc(26, 3),
    };

    public static Arc[] arcs_74_1 = {
        new Arc(1, -1),
        new Arc(32, 4),
    };

    public static Arc[] arcs_74_2 = {
        new Arc(1, -1),
        new Arc(26, 1),
    };

    public static Arc[] arcs_74_3 = {
        new Arc(1, -1),
        new Arc(32, 2),
    };

    public static Arc[] arcs_74_4 = {
        new Arc(1, -1),
        new Arc(26, 1),
    };

    public static State[] states_74 = {
        new State(1, GramInit.arcs_74_0),
        new State(2, GramInit.arcs_74_1),
        new State(2, GramInit.arcs_74_2),
        new State(2, GramInit.arcs_74_3),
        new State(2, GramInit.arcs_74_4),
    };

    public static Arc[] arcs_75_0 = {
        new Arc(34, 3),
        new Arc(26, 6),
        new Arc(50, 12),
    };

    public static Arc[] arcs_75_1 = {
        new Arc(1, -1),
        new Arc(34, 10),
        new Arc(26, 2),
    };

    public static Arc[] arcs_75_2 = {
        new Arc(27, 8),
    };

    public static Arc[] arcs_75_3 = {
        new Arc(107, 9),
    };

    public static Arc[] arcs_75_4 = {
        new Arc(1, -1),
        new Arc(34, 17),
        new Arc(26, 14),
    };

    public static Arc[] arcs_75_5 = {
        new Arc(1, -1),
        new Arc(26, 11),
        new Arc(50, 11),
    };

    public static Arc[] arcs_75_6 = {
        new Arc(1, -1),
        new Arc(163, 15),
        new Arc(27, 13),
        new Arc(32, 5),
    };

    public static Arc[] arcs_75_7 = {
        new Arc(1, -1),
        new Arc(32, 1),
    };

    public static Arc[] arcs_75_8 = {
        new Arc(26, 7),
    };

    public static Arc[] arcs_75_9 = {
        new Arc(1, -1),
        new Arc(163, 15),
        new Arc(32, 4),
    };

    public static Arc[] arcs_75_10 = {
        new Arc(107, 7),
    };

    public static Arc[] arcs_75_11 = {
        new Arc(1, -1),
        new Arc(32, 18),
    };

    public static Arc[] arcs_75_12 = {
        new Arc(1, -1),
        new Arc(163, 15),
        new Arc(32, 5),
    };

    public static Arc[] arcs_75_13 = {
        new Arc(26, 9),
    };

    public static Arc[] arcs_75_14 = {
        new Arc(27, 16),
    };

    public static Arc[] arcs_75_15 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_75_16 = {
        new Arc(26, 7),
    };

    public static Arc[] arcs_75_17 = {
        new Arc(107, 7),
    };

    public static Arc[] arcs_75_18 = {
        new Arc(1, -1),
        new Arc(26, 11),
        new Arc(50, 11),
    };

    public static State[] states_75 = {
        new State(3, GramInit.arcs_75_0),
        new State(3, GramInit.arcs_75_1),
        new State(1, GramInit.arcs_75_2),
        new State(1, GramInit.arcs_75_3),
        new State(3, GramInit.arcs_75_4),
        new State(3, GramInit.arcs_75_5),
        new State(4, GramInit.arcs_75_6),
        new State(2, GramInit.arcs_75_7),
        new State(1, GramInit.arcs_75_8),
        new State(3, GramInit.arcs_75_9),
        new State(1, GramInit.arcs_75_10),
        new State(2, GramInit.arcs_75_11),
        new State(3, GramInit.arcs_75_12),
        new State(1, GramInit.arcs_75_13),
        new State(1, GramInit.arcs_75_14),
        new State(1, GramInit.arcs_75_15),
        new State(1, GramInit.arcs_75_16),
        new State(1, GramInit.arcs_75_17),
        new State(3, GramInit.arcs_75_18),
    };

    public static Arc[] arcs_76_0 = {
        new Arc(167, 4),
    };

    public static Arc[] arcs_76_1 = {
        new Arc(28, 3),
    };

    public static Arc[] arcs_76_2 = {
        new Arc(14, 7),
        new Arc(15, 5),
    };

    public static Arc[] arcs_76_3 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_76_4 = {
        new Arc(23, 6),
    };

    public static Arc[] arcs_76_5 = {
        new Arc(27, 1),
    };

    public static Arc[] arcs_76_6 = {
        new Arc(27, 1),
        new Arc(13, 2),
    };

    public static Arc[] arcs_76_7 = {
        new Arc(15, 5),
    };

    public static State[] states_76 = {
        new State(1, GramInit.arcs_76_0),
        new State(1, GramInit.arcs_76_1),
        new State(2, GramInit.arcs_76_2),
        new State(1, GramInit.arcs_76_3),
        new State(1, GramInit.arcs_76_4),
        new State(1, GramInit.arcs_76_5),
        new State(2, GramInit.arcs_76_6),
        new State(1, GramInit.arcs_76_7),
    };

    public static Arc[] arcs_77_0 = {
        new Arc(168, 4),
    };

    public static Arc[] arcs_77_1 = {
        new Arc(1, -1),
        new Arc(32, 3),
    };

    public static Arc[] arcs_77_2 = {
        new Arc(1, -1),
        new Arc(168, 1),
    };

    public static Arc[] arcs_77_3 = {
        new Arc(1, -1),
        new Arc(168, 1),
    };

    public static Arc[] arcs_77_4 = {
        new Arc(1, -1),
        new Arc(32, 2),
    };

    public static State[] states_77 = {
        new State(1, GramInit.arcs_77_0),
        new State(2, GramInit.arcs_77_1),
        new State(2, GramInit.arcs_77_2),
        new State(2, GramInit.arcs_77_3),
        new State(2, GramInit.arcs_77_4),
    };

    public static Arc[] arcs_78_0 = {
        new Arc(34, 4),
        new Arc(26, 1),
        new Arc(33, 3),
    };

    public static Arc[] arcs_78_1 = {
        new Arc(1, -1),
        new Arc(163, 2),
        new Arc(31, 5),
    };

    public static Arc[] arcs_78_2 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_78_3 = {
        new Arc(26, 2),
    };

    public static Arc[] arcs_78_4 = {
        new Arc(26, 2),
    };

    public static Arc[] arcs_78_5 = {
        new Arc(26, 2),
    };

    public static State[] states_78 = {
        new State(3, GramInit.arcs_78_0),
        new State(3, GramInit.arcs_78_1),
        new State(1, GramInit.arcs_78_2),
        new State(1, GramInit.arcs_78_3),
        new State(1, GramInit.arcs_78_4),
        new State(1, GramInit.arcs_78_5),
    };

    public static Arc[] arcs_79_0 = {
        new Arc(163, 1),
        new Arc(170, 1),
    };

    public static Arc[] arcs_79_1 = {
        new Arc(1, -1),
    };

    public static State[] states_79 = {
        new State(2, GramInit.arcs_79_0),
        new State(1, GramInit.arcs_79_1),
    };

    public static Arc[] arcs_80_0 = {
        new Arc(100, 4),
    };

    public static Arc[] arcs_80_1 = {
        new Arc(1, -1),
        new Arc(169, 3),
    };

    public static Arc[] arcs_80_2 = {
        new Arc(101, 5),
    };

    public static Arc[] arcs_80_3 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_80_4 = {
        new Arc(65, 2),
    };

    public static Arc[] arcs_80_5 = {
        new Arc(111, 1),
    };

    public static State[] states_80 = {
        new State(1, GramInit.arcs_80_0),
        new State(2, GramInit.arcs_80_1),
        new State(1, GramInit.arcs_80_2),
        new State(1, GramInit.arcs_80_3),
        new State(1, GramInit.arcs_80_4),
        new State(1, GramInit.arcs_80_5),
    };

    public static Arc[] arcs_81_0 = {
        new Arc(96, 3),
    };

    public static Arc[] arcs_81_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_81_2 = {
        new Arc(1, -1),
        new Arc(169, 1),
    };

    public static Arc[] arcs_81_3 = {
        new Arc(113, 2),
    };

    public static State[] states_81 = {
        new State(1, GramInit.arcs_81_0),
        new State(1, GramInit.arcs_81_1),
        new State(2, GramInit.arcs_81_2),
        new State(1, GramInit.arcs_81_3),
    };

    public static Arc[] arcs_82_0 = {
        new Arc(23, 1),
    };

    public static Arc[] arcs_82_1 = {
        new Arc(1, -1),
    };

    public static State[] states_82 = {
        new State(1, GramInit.arcs_82_0),
        new State(1, GramInit.arcs_82_1),
    };

    public static Arc[] arcs_83_0 = {
        new Arc(172, 1),
    };

    public static Arc[] arcs_83_1 = {
        new Arc(1, -1),
        new Arc(173, 2),
    };

    public static Arc[] arcs_83_2 = {
        new Arc(1, -1),
    };

    public static State[] states_83 = {
        new State(1, GramInit.arcs_83_0),
        new State(2, GramInit.arcs_83_1),
        new State(1, GramInit.arcs_83_2),
    };

    public static Arc[] arcs_84_0 = {
        new Arc(76, 2),
        new Arc(9, 1),
    };

    public static Arc[] arcs_84_1 = {
        new Arc(1, -1),
    };

    public static Arc[] arcs_84_2 = {
        new Arc(26, 1),
    };

    public static State[] states_84 = {
        new State(2, GramInit.arcs_84_0),
        new State(1, GramInit.arcs_84_1),
        new State(1, GramInit.arcs_84_2),
    };

    public final static int[] acceptLabel_0 = {160, 100, 66, 2, 141, 89, 75, 72, 73, 120, 161, 155, 82, 162, 64, 140, 105, 167, 153, 102, 74, 22, 13, 11, 146, 23, 33, 88, 158, 115, 99, 96, 79, 87, 159, 149, 76, 21, 172, };
    public final static int[] acceptLabel_1 = {160, 100, 66, 2, 141, 89, 75, 72, 73, 120, 161, 155, 7, 82, 162, 64, 140, 105, 167, 153, 102, 74, 22, 13, 11, 146, 23, 33, 88, 158, 115, 99, 96, 79, 87, 159, 149, 76, 21, 172, };
    public final static int[] acceptLabel_2 = {160, 153, 13, 146, 141, 23, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_3 = {11, };
    public final static int[] acceptLabel_4 = {11, };
    public final static int[] acceptLabel_5 = {11, };
    public final static int[] acceptLabel_6 = {21, };
    public final static int[] acceptLabel_7 = {22, };
    public final static int[] acceptLabel_8 = {13, };
    public final static int[] acceptLabel_9 = {34, 23, 33, };
    public final static int[] acceptLabel_10 = {23, };
    public final static int[] acceptLabel_11 = {34, 23, 33, };
    public final static int[] acceptLabel_12 = {23, };
    public final static int[] acceptLabel_13 = {160, 100, 66, 141, 89, 75, 72, 73, 120, 161, 155, 82, 162, 64, 140, 105, 167, 153, 102, 74, 22, 13, 11, 146, 23, 33, 88, 158, 115, 99, 96, 79, 87, 159, 149, 76, 21, 172, };
    public final static int[] acceptLabel_14 = {160, 66, 141, 89, 75, 72, 73, 120, 161, 155, 82, 162, 64, 140, 153, 74, 13, 146, 23, 33, 88, 158, 115, 79, 87, 159, 149, 76, 172, };
    public final static int[] acceptLabel_15 = {160, 66, 141, 89, 75, 72, 73, 120, 161, 155, 82, 162, 64, 140, 153, 74, 13, 146, 23, 33, 88, 158, 115, 79, 87, 159, 149, 76, 172, };
    public final static int[] acceptLabel_16 = {160, 153, 13, 146, 141, 23, 33, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_17 = {160, 153, 13, 146, 141, 23, 33, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_18 = {56, 61, 62, 59, 55, 54, 57, 60, 52, 51, 58, 53, 63, };
    public final static int[] acceptLabel_19 = {64, };
    public final static int[] acceptLabel_20 = {66, };
    public final static int[] acceptLabel_21 = {73, 74, 172, 75, 72, };
    public final static int[] acceptLabel_22 = {72, };
    public final static int[] acceptLabel_23 = {73, };
    public final static int[] acceptLabel_24 = {74, };
    public final static int[] acceptLabel_25 = {172, };
    public final static int[] acceptLabel_26 = {75, };
    public final static int[] acceptLabel_27 = {79, 76, };
    public final static int[] acceptLabel_28 = {79, };
    public final static int[] acceptLabel_29 = {76, };
    public final static int[] acceptLabel_30 = {23, };
    public final static int[] acceptLabel_31 = {23, };
    public final static int[] acceptLabel_32 = {23, };
    public final static int[] acceptLabel_33 = {23, };
    public final static int[] acceptLabel_34 = {23, };
    public final static int[] acceptLabel_35 = {87, };
    public final static int[] acceptLabel_36 = {88, };
    public final static int[] acceptLabel_37 = {89, };
    public final static int[] acceptLabel_38 = {99, 105, 100, 167, 96, 102, 22, 11, 21, };
    public final static int[] acceptLabel_39 = {21, };
    public final static int[] acceptLabel_40 = {96, };
    public final static int[] acceptLabel_41 = {99, };
    public final static int[] acceptLabel_42 = {100, };
    public final static int[] acceptLabel_43 = {102, };
    public final static int[] acceptLabel_44 = {105, };
    public final static int[] acceptLabel_45 = {160, 153, 13, 146, 141, 23, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_46 = {108, };
    public final static int[] acceptLabel_47 = {160, 66, 2, 141, 89, 75, 72, 73, 120, 161, 155, 82, 162, 64, 140, 153, 74, 13, 146, 23, 33, 88, 158, 115, 79, 87, 159, 149, 76, 172, };
    public final static int[] acceptLabel_48 = {160, 153, 13, 146, 141, 23, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_49 = {160, 153, 13, 146, 141, 23, 115, 158, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_50 = {115, };
    public final static int[] acceptLabel_51 = {115, };
    public final static int[] acceptLabel_52 = {160, 153, 13, 146, 141, 23, 158, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_53 = {160, 153, 13, 146, 141, 23, 158, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_54 = {160, 153, 13, 146, 141, 23, 158, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_55 = {160, 153, 13, 146, 141, 23, 158, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_56 = {120, 127, 101, 128, 123, 125, 129, 124, 126, };
    public final static int[] acceptLabel_57 = {33, };
    public final static int[] acceptLabel_58 = {160, 153, 13, 146, 141, 23, 158, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_59 = {160, 153, 13, 146, 141, 23, 158, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_60 = {160, 153, 13, 146, 141, 23, 158, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_61 = {160, 153, 13, 146, 141, 23, 158, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_62 = {160, 153, 13, 146, 141, 23, 158, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_63 = {160, 153, 13, 146, 141, 23, 158, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_64 = {160, 153, 13, 146, 141, 23, 158, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_65 = {158, 160, 153, 161, 155, 159, 13, 149, 82, 162, 23, };
    public final static int[] acceptLabel_66 = {158, 160, 153, 161, 155, 159, 13, 149, 82, 162, 23, };
    public final static int[] acceptLabel_67 = {158, 160, 153, 161, 155, 159, 13, 82, 162, 23, };
    public final static int[] acceptLabel_68 = {160, 153, 13, 146, 141, 23, 33, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_69 = {153, 81, 13, };
    public final static int[] acceptLabel_70 = {160, 153, 27, 13, 146, 141, 23, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_71 = {160, 153, 27, 13, 146, 141, 23, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_72 = {27, };
    public final static int[] acceptLabel_73 = {160, 153, 13, 146, 141, 23, 33, 158, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_74 = {160, 153, 13, 146, 141, 23, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_75 = {160, 153, 34, 13, 146, 141, 23, 33, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_76 = {167, };
    public final static int[] acceptLabel_77 = {160, 153, 34, 13, 146, 141, 23, 33, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_78 = {160, 153, 34, 13, 146, 141, 23, 33, 158, 115, 120, 161, 155, 159, 149, 82, 162, 140, };
    public final static int[] acceptLabel_79 = {100, 96, };
    public final static int[] acceptLabel_80 = {100, };
    public final static int[] acceptLabel_81 = {96, };
    public final static int[] acceptLabel_82 = {23, };
    public final static int[] acceptLabel_83 = {172, };
    public final static int[] acceptLabel_84 = {160, 153, 13, 146, 141, 23, 158, 115, 120, 161, 155, 159, 76, 149, 82, 162, 140, };
    public static DFA[] dfas = {
        new DFA(DFAType.single_input, 0, 3, GramInit.states_0, acceptLabel_0),
        new DFA(DFAType.file_input, 0, 3, GramInit.states_1, acceptLabel_1),
        new DFA(DFAType.eval_input, 0, 4, GramInit.states_2, acceptLabel_2),
        new DFA(DFAType.decorator, 0, 7, GramInit.states_3, acceptLabel_4),
        new DFA(DFAType.decorators, 0, 2, GramInit.states_4, acceptLabel_4),
        new DFA(DFAType.decorated, 0, 3, GramInit.states_5, acceptLabel_5),
        new DFA(DFAType.async_funcdef, 0, 3, GramInit.states_6, acceptLabel_6),
        new DFA(DFAType.funcdef, 0, 8, GramInit.states_7, acceptLabel_7),
        new DFA(DFAType.parameters, 0, 4, GramInit.states_8, acceptLabel_8),
        new DFA(DFAType.typedargslist, 0, 34, GramInit.states_9, acceptLabel_9),
        new DFA(DFAType.tfpdef, 0, 4, GramInit.states_10, acceptLabel_10),
        new DFA(DFAType.varargslist, 0, 34, GramInit.states_11, acceptLabel_11),
        new DFA(DFAType.vfpdef, 0, 2, GramInit.states_12, acceptLabel_12),
        new DFA(DFAType.stmt, 0, 2, GramInit.states_13, acceptLabel_13),
        new DFA(DFAType.simple_stmt, 0, 6, GramInit.states_14, acceptLabel_14),
        new DFA(DFAType.small_stmt, 0, 2, GramInit.states_15, acceptLabel_15),
        new DFA(DFAType.expr_stmt, 0, 7, GramInit.states_16, acceptLabel_16),
        new DFA(DFAType.testlist_star_expr, 0, 5, GramInit.states_17, acceptLabel_68),
        new DFA(DFAType.augassign, 0, 2, GramInit.states_18, acceptLabel_18),
        new DFA(DFAType.del_stmt, 0, 3, GramInit.states_19, acceptLabel_19),
        new DFA(DFAType.pass_stmt, 0, 2, GramInit.states_20, acceptLabel_20),
        new DFA(DFAType.flow_stmt, 0, 2, GramInit.states_21, acceptLabel_21),
        new DFA(DFAType.break_stmt, 0, 2, GramInit.states_22, acceptLabel_22),
        new DFA(DFAType.continue_stmt, 0, 2, GramInit.states_23, acceptLabel_23),
        new DFA(DFAType.return_stmt, 0, 3, GramInit.states_24, acceptLabel_24),
        new DFA(DFAType.yield_stmt, 0, 2, GramInit.states_25, acceptLabel_83),
        new DFA(DFAType.raise_stmt, 0, 5, GramInit.states_26, acceptLabel_26),
        new DFA(DFAType.import_stmt, 0, 2, GramInit.states_27, acceptLabel_27),
        new DFA(DFAType.import_name, 0, 3, GramInit.states_28, acceptLabel_28),
        new DFA(DFAType.import_from, 0, 8, GramInit.states_29, acceptLabel_29),
        new DFA(DFAType.import_as_name, 0, 4, GramInit.states_30, acceptLabel_32),
        new DFA(DFAType.dotted_as_name, 0, 4, GramInit.states_31, acceptLabel_34),
        new DFA(DFAType.import_as_names, 0, 5, GramInit.states_32, acceptLabel_32),
        new DFA(DFAType.dotted_as_names, 0, 5, GramInit.states_33, acceptLabel_33),
        new DFA(DFAType.dotted_name, 0, 5, GramInit.states_34, acceptLabel_34),
        new DFA(DFAType.global_stmt, 0, 6, GramInit.states_35, acceptLabel_35),
        new DFA(DFAType.nonlocal_stmt, 0, 6, GramInit.states_36, acceptLabel_36),
        new DFA(DFAType.assert_stmt, 0, 5, GramInit.states_37, acceptLabel_37),
        new DFA(DFAType.compound_stmt, 0, 2, GramInit.states_38, acceptLabel_38),
        new DFA(DFAType.async_stmt, 0, 3, GramInit.states_39, acceptLabel_39),
        new DFA(DFAType.if_stmt, 0, 15, GramInit.states_40, acceptLabel_40),
        new DFA(DFAType.while_stmt, 0, 8, GramInit.states_41, acceptLabel_41),
        new DFA(DFAType.for_stmt, 0, 10, GramInit.states_42, acceptLabel_42),
        new DFA(DFAType.try_stmt, 0, 17, GramInit.states_43, acceptLabel_43),
        new DFA(DFAType.with_stmt, 0, 8, GramInit.states_44, acceptLabel_44),
        new DFA(DFAType.with_item, 0, 4, GramInit.states_45, acceptLabel_74),
        new DFA(DFAType.except_clause, 0, 5, GramInit.states_46, acceptLabel_46),
        new DFA(DFAType.suite, 0, 5, GramInit.states_47, acceptLabel_47),
        new DFA(DFAType.test, 0, 6, GramInit.states_48, acceptLabel_48),
        new DFA(DFAType.test_nocond, 0, 2, GramInit.states_49, acceptLabel_49),
        new DFA(DFAType.lambdef, 0, 5, GramInit.states_50, acceptLabel_50),
        new DFA(DFAType.lambdef_nocond, 0, 5, GramInit.states_51, acceptLabel_51),
        new DFA(DFAType.or_test, 0, 5, GramInit.states_52, acceptLabel_52),
        new DFA(DFAType.and_test, 0, 5, GramInit.states_53, acceptLabel_53),
        new DFA(DFAType.not_test, 0, 3, GramInit.states_54, acceptLabel_54),
        new DFA(DFAType.comparison, 0, 5, GramInit.states_55, acceptLabel_55),
        new DFA(DFAType.comp_op, 0, 4, GramInit.states_56, acceptLabel_56),
        new DFA(DFAType.star_expr, 0, 3, GramInit.states_57, acceptLabel_57),
        new DFA(DFAType.expr, 0, 5, GramInit.states_58, acceptLabel_58),
        new DFA(DFAType.xor_expr, 0, 5, GramInit.states_59, acceptLabel_59),
        new DFA(DFAType.and_expr, 0, 5, GramInit.states_60, acceptLabel_60),
        new DFA(DFAType.shift_expr, 0, 5, GramInit.states_61, acceptLabel_61),
        new DFA(DFAType.arith_expr, 0, 5, GramInit.states_62, acceptLabel_62),
        new DFA(DFAType.term, 0, 5, GramInit.states_63, acceptLabel_63),
        new DFA(DFAType.factor, 0, 3, GramInit.states_64, acceptLabel_64),
        new DFA(DFAType.power, 0, 4, GramInit.states_65, acceptLabel_65),
        new DFA(DFAType.atom_expr, 0, 4, GramInit.states_66, acceptLabel_66),
        new DFA(DFAType.atom, 0, 9, GramInit.states_67, acceptLabel_67),
        new DFA(DFAType.testlist_comp, 0, 6, GramInit.states_68, acceptLabel_68),
        new DFA(DFAType.trailer, 0, 7, GramInit.states_69, acceptLabel_69),
        new DFA(DFAType.subscriptlist, 0, 5, GramInit.states_70, acceptLabel_70),
        new DFA(DFAType.subscript, 0, 5, GramInit.states_71, acceptLabel_71),
        new DFA(DFAType.sliceop, 0, 3, GramInit.states_72, acceptLabel_72),
        new DFA(DFAType.exprlist, 0, 5, GramInit.states_73, acceptLabel_73),
        new DFA(DFAType.testlist, 0, 5, GramInit.states_74, acceptLabel_74),
        new DFA(DFAType.dictorsetmaker, 0, 19, GramInit.states_75, acceptLabel_75),
        new DFA(DFAType.classdef, 0, 8, GramInit.states_76, acceptLabel_76),
        new DFA(DFAType.arglist, 0, 5, GramInit.states_77, acceptLabel_77),
        new DFA(DFAType.argument, 0, 6, GramInit.states_78, acceptLabel_78),
        new DFA(DFAType.comp_iter, 0, 2, GramInit.states_79, acceptLabel_79),
        new DFA(DFAType.comp_for, 0, 6, GramInit.states_80, acceptLabel_80),
        new DFA(DFAType.comp_if, 0, 4, GramInit.states_81, acceptLabel_81),
        new DFA(DFAType.encoding_decl, 0, 2, GramInit.states_82, acceptLabel_82),
        new DFA(DFAType.yield_expr, 0, 3, GramInit.states_83, acceptLabel_83),
        new DFA(DFAType.yield_arg, 0, 3, GramInit.states_84, acceptLabel_84),
    };

    public static Label[] labels = {
        new Label(0),
        new Label(-1),
        new Label(TokState.NEWLINE, null),
        new Label(14),
        new Label(38),
        new Label(1),
        new Label(13),
        new Label(TokState.ENDMARKER, null),
        new Label(2),
        new Label(74),
        new Label(3),
        new Label(TokState.AT, null),
        new Label(34),
        new Label(TokState.LPAR, null),
        new Label(77),
        new Label(TokState.RPAR, null),
        new Label(4),
        new Label(5),
        new Label(76),
        new Label(7),
        new Label(6),
        new Label(TokState.ASYNC, null),
        new Label(TokState.NAME, "def"),
        new Label(TokState.NAME, null),
        new Label(8),
        new Label(TokState.RARROW, null),
        new Label(48),
        new Label(TokState.COLON, null),
        new Label(47),
        new Label(9),
        new Label(10),
        new Label(TokState.EQUAL, null),
        new Label(TokState.COMMA, null),
        new Label(TokState.STAR, null),
        new Label(TokState.DOUBLESTAR, null),
        new Label(11),
        new Label(12),
        new Label(15),
        new Label(TokState.SEMI, null),
        new Label(16),
        new Label(19),
        new Label(20),
        new Label(21),
        new Label(27),
        new Label(35),
        new Label(36),
        new Label(37),
        new Label(17),
        new Label(18),
        new Label(83),
        new Label(57),
        new Label(TokState.PLUSEQUAL, null),
        new Label(TokState.MINEQUAL, null),
        new Label(TokState.STAREQUAL, null),
        new Label(TokState.ATEQUAL, null),
        new Label(TokState.SLASHEQUAL, null),
        new Label(TokState.PERCENTEQUAL, null),
        new Label(TokState.AMPEREQUAL, null),
        new Label(TokState.VBAREQUAL, null),
        new Label(TokState.CIRCUMFLEXEQUAL, null),
        new Label(TokState.LEFTSHIFTEQUAL, null),
        new Label(TokState.RIGHTSHIFTEQUAL, null),
        new Label(TokState.DOUBLESTAREQUAL, null),
        new Label(TokState.DOUBLESLASHEQUAL, null),
        new Label(TokState.NAME, "del"),
        new Label(73),
        new Label(TokState.NAME, "pass"),
        new Label(22),
        new Label(23),
        new Label(24),
        new Label(26),
        new Label(25),
        new Label(TokState.NAME, "break"),
        new Label(TokState.NAME, "continue"),
        new Label(TokState.NAME, "return"),
        new Label(TokState.NAME, "raise"),
        new Label(TokState.NAME, "from"),
        new Label(28),
        new Label(29),
        new Label(TokState.NAME, "import"),
        new Label(33),
        new Label(TokState.DOT, null),
        new Label(TokState.ELLIPSIS, null),
        new Label(32),
        new Label(30),
        new Label(TokState.NAME, "as"),
        new Label(31),
        new Label(TokState.NAME, "global"),
        new Label(TokState.NAME, "nonlocal"),
        new Label(TokState.NAME, "assert"),
        new Label(40),
        new Label(41),
        new Label(42),
        new Label(43),
        new Label(44),
        new Label(39),
        new Label(TokState.NAME, "if"),
        new Label(TokState.NAME, "elif"),
        new Label(TokState.NAME, "else"),
        new Label(TokState.NAME, "while"),
        new Label(TokState.NAME, "for"),
        new Label(TokState.NAME, "in"),
        new Label(TokState.NAME, "try"),
        new Label(46),
        new Label(TokState.NAME, "finally"),
        new Label(TokState.NAME, "with"),
        new Label(45),
        new Label(58),
        new Label(TokState.NAME, "except"),
        new Label(TokState.INDENT, null),
        new Label(TokState.DEDENT, null),
        new Label(52),
        new Label(50),
        new Label(49),
        new Label(51),
        new Label(TokState.NAME, "lambda"),
        new Label(53),
        new Label(TokState.NAME, "or"),
        new Label(54),
        new Label(TokState.NAME, "and"),
        new Label(TokState.NAME, "not"),
        new Label(55),
        new Label(56),
        new Label(TokState.LESS, null),
        new Label(TokState.GREATER, null),
        new Label(TokState.EQEQUAL, null),
        new Label(TokState.GREATEREQUAL, null),
        new Label(TokState.LESSEQUAL, null),
        new Label(TokState.NOTEQUAL, null),
        new Label(TokState.NAME, "is"),
        new Label(59),
        new Label(TokState.VBAR, null),
        new Label(60),
        new Label(TokState.CIRCUMFLEX, null),
        new Label(61),
        new Label(TokState.AMPER, null),
        new Label(62),
        new Label(TokState.LEFTSHIFT, null),
        new Label(TokState.RIGHTSHIFT, null),
        new Label(63),
        new Label(TokState.PLUS, null),
        new Label(TokState.MINUS, null),
        new Label(64),
        new Label(TokState.SLASH, null),
        new Label(TokState.PERCENT, null),
        new Label(TokState.DOUBLESLASH, null),
        new Label(TokState.TILDE, null),
        new Label(65),
        new Label(66),
        new Label(TokState.AWAIT, null),
        new Label(67),
        new Label(69),
        new Label(68),
        new Label(TokState.LSQB, null),
        new Label(TokState.RSQB, null),
        new Label(TokState.LBRACE, null),
        new Label(75),
        new Label(TokState.RBRACE, null),
        new Label(TokState.NUMBER, null),
        new Label(TokState.STRING, null),
        new Label(TokState.NAME, "None"),
        new Label(TokState.NAME, "True"),
        new Label(TokState.NAME, "False"),
        new Label(80),
        new Label(70),
        new Label(71),
        new Label(72),
        new Label(TokState.NAME, "class"),
        new Label(78),
        new Label(79),
        new Label(81),
        new Label(82),
        new Label(TokState.NAME, "yield"),
        new Label(84),
    };

    public static Grammar grammar = new Grammar(85, GramInit.dfas, 174, GramInit.labels, 0);
};
