...
public class Token
{
	...
	public String toString()
	{
		...
		if(this.state == TokState.NEWLINE || 
		...
		return this.lineNo + " " + this.state.toString() + ": " + this.buf.substring(this.start, this.end);
	}
	...
	private boolean setIndentifier()
	{
		...
		if(!blankline && this.level == 0)
		{
			...
			else if(col > this.indstack[this.indent])
			{
				...
				{
					throw new PyExceptions("IndentationError: too many levels of indentation");
				}
				this.pendin++;
	...
	private boolean checkName(char c)
	{
		...
		while(true)
		{
			if(!(saw_b || saw_u) && (c == 'b' || c == 'B')) saw_b = true;
			else if(!(saw_u || saw_b || saw_r) && (c == 'u' || c == 'U')) saw_u = true;
			else if(!(saw_r || saw_u) && (c == 'r' || c == 'R')) saw_r = true;
			else break;
			c = this.nextC();
			if(c == '\'' || c == '"')
			{
