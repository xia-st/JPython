...
class EmitVisitor(asdl.VisitorBase):
    ...
    def open(self, name, refersToPythonNode=1, useDataOutput=0):
        ...
        if refersToPythonNode:
            print >> self.file, 'import pers.xia.jpython.parser.PythonNode;'
        if useDataOutput:
...
class JavaVisitor(EmitVisitor):
    ...
    def sum_with_constructor(self, sum, name, depth):
        ...
        self.emit("public abstract class %(name)sType extends PythonNode {" %
                  locals(), depth)
    ...
    def visitProduct(self, product, name, depth):
        ...
        self.emit("public class %(name)sType extends PythonNode {" % locals(),
                  depth)
        for f in product.fields:
            self.visit(f, depth + 1)
        self.emit("", depth)
    ...
    def javaMethods(self, type, clsname, ctorname, fields, depth):
        ...
        if fpargs:
            fpargs += ", "
        self.emit("public %s(%sPythonNode parent) {" %
...
class VisitorVisitor(EmitVisitor):
    ...
    def visitModule(self, mod):
        ...
        self.emit("abstract protected Object unhandled_node(PythonNode node) throws Exception;", 1)
        self.emit("abstract public void traverse(PythonNode node) throws Exception;", 1)
        self.emit('}', 0)
        self.close()
